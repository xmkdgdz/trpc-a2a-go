...
type TaskHandle interface {
	...
	UpdateStatus(state TaskState, msg *Message) error

	// AddArtifact adds a new artifact to the task.
	// Returns an error if the task cannot be found or updated.
	AddArtifact(artifact Artifact) error
}
...
type TaskProcessor interface {
	...
	Process(ctx context.Context, taskID string, initialMsg Message, handle TaskHandle) error
}
...
type TaskManager interface {
	...
	OnSendTask(ctx context.Context, request SendTaskParams) (*Task, error)
	...
	OnSendTaskSubscribe(ctx context.Context, request SendTaskParams) (<-chan TaskEvent, error)

	// OnGetTask handles a request corresponding to the 'tasks/get' RPC method.
	// It retrieves the current state of an existing task.
	OnGetTask(ctx context.Context, params TaskQueryParams) (*Task, error)
	...
	OnCancelTask(ctx context.Context, params TaskIDParams) (*Task, error)
	...
	OnPushNotificationSet(ctx context.Context, params TaskPushNotificationConfig) (*TaskPushNotificationConfig, error)
	...
	OnPushNotificationGet(ctx context.Context, params TaskIDParams) (*TaskPushNotificationConfig, error)
	...
	OnResubscribe(ctx context.Context, params TaskIDParams) (<-chan TaskEvent, error)
}
